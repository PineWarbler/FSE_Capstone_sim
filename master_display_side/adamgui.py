...
for name, ch_entry in my_channel_entries.channels.items():
    ...
    ao_label_objects[name] = current_label

    # save_text_button = ctk.CTkButton(frame, text="Send", fg_color="green",
                                      # command=print(name))
    save_text_button = ctk.CTkButton(frame, text="Send", fg_color="green",
                                      command=lambda n=name, e=input_value_entry: place_single(name=n,entry=e))
